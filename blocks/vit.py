...
from .common import LayerNormProxy
import math
...
class Mlp(nn.Module):
    def __init__(self, in_features, hidden_features=None, out_features=None, act_layer=nn.GELU, drop=0., use_conv=False):
        super().__init__()
        ...
        if use_conv:
            self.linear1 = nn.Conv2d(in_features, hidden_features, 1, 1, 0)
            self.linear2 = nn.Conv2d(hidden_features, out_features, 1, 1, 0) 
            self.dwc = nn.Conv2d(hidden_features, hidden_features, 3, 1, 1, groups=hidden_features)
        else:
...
class LocalAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        x_total = rearrange(x, 'b c (r1 h1) (r2 w1) -> b (r1 r2) (h1 w1) c', h1=self.window_size[0], w1=self.window_size[1]) # B x Nr x Ws x C
    ...
    def __init__(self, dim, heads, window_size, attn_drop, proj_drop, shift_size, fmap_size):
        ...
        for h in h_slices:
            for w in w_slices:
                img_mask[h, w] = cnt
                cnt += 1
        mask_windows = rearrange(img_mask, '(r1 h1) (r2 w1) -> (r1 r2) (h1 w1)', h1=self.window_size[0],w1=self.window_size[1])
...
class IBIBlock(nn.Module):
    def __init__(self, fmap_size, window_size,
                 dim_in, dim_embed, depths, stage_spec, 
                 heads, attn_drop, proj_drop, expansion_mlp,
                 drop, drop_path_rate, use_dwc_mlp,
                #  n_groups, use_pe, stride, offset_range_factor, stage_idx, dwc_pe, no_off, fixed_pe,
                 ):
        super().__init__()
        drop_path_rate = to_2tuple(drop_path_rate) if isinstance(drop_path_rate, (int, float)) else drop_path_rate
        ...
        self.mlps = nn.ModuleList([Mlp(dim_embed, expansion_mlp*dim_embed, dim_embed, drop=drop, use_conv=use_dwc_mlp) for _ in range(depths)])
        self.attns = nn.ModuleList()
        self.drop_path = nn.ModuleList()
        for i in range(depths):
            if stage_spec[i] == 'L':
                att = LocalAttention(dim_embed, heads, window_size, attn_drop, proj_drop)
            # elif stage_spec[i] == 'D':
            #     att = DAttentionBaseline(fmap_size, fmap_size, heads, 
            #         hc, n_groups, attn_drop, proj_drop, 
            #         stride, offset_range_factor, use_pe, dwc_pe, 
            #         no_off, fixed_pe, stage_idx)
            elif stage_spec[i] == 'S':
                shift_size = math.ceil(window_size / 2)
                att = ShiftWindowAttention(dim_embed, heads, window_size, attn_drop, proj_drop, shift_size, fmap_size)
            else:
                raise NotImplementedError(f'Spec: {stage_spec[i]} is not supported.')
            self.attns.append(att)
